...
const MobileMenu: React.FC<IProps> = ({
  ...
  return (
    <div className={css.wrap}>
      <button className={css.openMenuBtn} onClick={() => setIsOpen(true)}>
        <MenuIcon
          className={cn(css.openMenuIcon, style === "light" && css.light)}
        />
      </button>
      {/* MENU */}
      {isOpen && (
        <div className={css.backdrop}>
          <div className={css.modal}>
            ...
            <PrismicNextLink className={css.bookingBtn} field={button.link}>
              {button.label}
            </PrismicNextLink>
            <ul className={css.contactsList}>
              {contacts.map(({ icon, value }, index) => (
                <li key={index} className={css.contactItem}>
                  ...
                  {icon === "phone" && (
                    <PhoneIcon className={css.contactIcon} />
                  )}
                  {value}
                </li>
              ))}
  ...
  return (
    ...
    >
      <path
        ...
        strokeWidth="1.5"
        d="M10.208 10.208l14.583 14.584m-14.583 0l14.583-14.584"
      ></path>
    </svg>
  );
}
